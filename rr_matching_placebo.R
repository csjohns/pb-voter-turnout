...
voterfile <- voterfile %>% 
  mutate_at(vars(starts_with("g_"), starts_with("p_")), replace_na, 0) %>% 
  mutate(age = year(Sys.Date()) - year(DoB),
         g_early = g_2000 + g_2001 + g_2002 + g_2003 + g_2004 + g_2005 + g_2006 + g_2007,
         p_early = p_2000 + p_2001 + p_2002 + p_2003 + p_2004 + p_2005 + p_2006 + p_2007) 
gc()
...
# saveRDS(voterfile, "voterfile_placebo.rds")
# voterfile <- readRDS("voterfile_placebo.rds")
...
district_covar <- district_covar %>% 
  rename(NYCCD = councildist)

voterfile <- district_covar %>% 
  select(NYCCD, white_pct, black_pct, pct18, noneng_pct, votespop) %>% 
...
#renaming pbdistrict to pb to ease code copy
voterfile <- rename(voterfile,
                    pb = pbdistrict)
