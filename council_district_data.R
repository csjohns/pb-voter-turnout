...
ccmstats <- ccmstats %>% 
  select(district, year, incumbent, competitivejenks) %>% 
  filter(year %in% c(2009,2013,2017)) %>% 
  pivot_wider(names_from = year, values_from = c("incumbent", "competitivejenks")) #%>%
...
saveRDS(ccdist_combined, "data/cleaned_R_results/council_districts.rds")
